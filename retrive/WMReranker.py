...
from llama_index.postprocessor.flag_embedding_reranker import FlagEmbeddingReranker     #也可以从legacy中import，但是这里还是安装了以前版本的文件
...
class WMReranker(Retrive):
    ...
    def colbert_rerank(self, documents: List[Document],similarity_cutoff=0.6) -> List[Document]:
        query_engine = RetrieverQueryEngine(
            retriever=self.retriever,
            response_synthesizer=None,
            node_postprocessors=[SimilarityPostprocessor(similarity_cutoff=similarity_cutoff),self.colbert_reranker],
        )
        reranked_response = query_engine.query("")  # Empty query
        return reranked_response
